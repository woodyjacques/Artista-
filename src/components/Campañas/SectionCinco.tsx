...
function SectionCinco({ title, subtitle, description }: SectionTresProps) {
    return (
        <div className="font-arima flex flex-col lg:flex-row items-center justify-center gap-8 px-8 lg:px-20 py-8">

            <div className="lg:w-1/2 w-full flex flex-col items-start justify-center text-gray-800 text-left space-y-4">
                <div className="flex items-center space-x-3">
                    <img src={homeCampaña} alt="Icono de campaña" className="w-10 h-10 bg-blue-800 p-2 rounded-full" />
                    <h2 className="text-2xl font-semibold">{title}</h2>
                </div>
                <h3 className="text-md font-medium text-gray-500">{subtitle}</h3>
